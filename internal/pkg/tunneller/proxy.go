...
func (t *Tunnel) rewriteLocation(loc, prefix string) (string, error) {
	...
	if u.IsAbs() {
		t.logger.Debug("got absolute URL for Location header, making non-absolute", "location", u.String())
		u.Scheme = ""
		u.Host = ""
	}
	...
	if strings.HasPrefix(u.Path, "/") &&
		...
		u.Path = p
		t.logger.Debug("rewrote Location header", "location", u.String())
		return u.String(), nil
	}

	t.logger.Debug("no rewrite of Location header required", "location", u.String())
	return u.String(), nil
}
